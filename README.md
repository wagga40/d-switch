...
**Trigger a cursor jump with either:**

- **Cmd+Shift+M** — keyboard shortcut
- **Four-finger tap** on the trackpad

Both triggers do the same thing: move the cursor to the next display, **focus the topmost window** on that display, and show a brief ring so you can instantly locate the cursor.

With two displays, each trigger toggles between them. With three or more, it cycles through displays in spatial order (left-to-right, then top-to-bottom).
...
The app bundle is created at `build/D-Switch.app`. You can also copy it to `/Applications` and launch it from there.

## Permissions

**Core functionality works without special permissions.** The global hotkey (Cmd+Shift+M) and cursor movement use APIs that don't require accessibility access. The four-finger tap uses the MultitouchSupport framework, which also does not require explicit permissions.

For the best experience, grant **Accessibility** permission in **System Settings > Privacy & Security > Accessibility**. This allows D-Switch to post a mouse-moved event after warping the cursor, which helps some apps register the new cursor position immediately.

If accessibility is not granted:
- The cursor still moves correctly
- The hotkey and four-finger tap both still work
- Some apps may briefly show the cursor in the old position until you move it

The app checks accessibility status on launch and logs the result. It never crashes due to missing permissions.
...
This means the cycle order matches the physical arrangement of your displays. The ordering is recalculated each time the shortcut is triggered, so it adapts if you rearrange displays.
...
**Toggle:** Click the menu bar icon → **Auto-Focus Window** (checkmark = on). The setting persists via UserDefaults. When off, the cursor always goes to the display center.
...
- **Auto-Focus Window** — toggle automatic window focusing (on by default)
- **Quit** — exit D-Switch
